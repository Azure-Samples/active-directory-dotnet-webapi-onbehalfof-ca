...
#> 
   ...
   # Create the downstreamService AAD application
   Write-Host "Creating the AAD appplication (DownstreamService-OBO-CA)"
   $downstreamServiceAadApplication = New-AzureADApplication -DisplayName "DownstreamService-OBO-CA" `
                                                             -HomePage "https://localhost:44302/" `
                                                             -IdentifierUris "https://$tenantName/DownstreamService-OBO-CA" `
                                                             -PublicClient $False


   $currentAppId = $downstreamServiceAadApplication.AppId
   $downstreamServiceServicePrincipal = New-AzureADServicePrincipal -AppId $currentAppId -Tags {WindowsAzureActiveDirectoryIntegratedApp}
   Write-Host "Done."

   # URL of the AAD application in the Azure portal
   $downstreamServicePortalUrl = "https://portal.azure.com/#@"+$tenantName+"/blade/Microsoft_AAD_IAM/ApplicationBlade/appId/"+$downstreamServiceAadApplication.AppId+"/objectId/"+$downstreamServiceAadApplication.ObjectId
   Add-Content -Value "<tr><td>downstreamService</td><td>$currentAppId</td><td><a href='$downstreamServicePortalUrl'>DownstreamService-OBO-CA</a></td></tr>" -Path createdApps.html

   # Create the service AAD application
   Write-Host "Creating the AAD appplication (TodoListService-OBO-CA)"
   # Get a 2 years application key for the service Application
   $pw = ComputePassword
   $fromDate = [DateTime]::Now;
   $key = CreateAppKey -fromDate $fromDate -durationInYears 2 -pw $pw
   $serviceAppKey = $pw
   $serviceAadApplication = New-AzureADApplication -DisplayName "TodoListService-OBO-CA" `
                                                   -HomePage "https://localhost:44321/" `
                                                   -IdentifierUris "https://$tenantName/TodoListService-OBO-CA" `
   ...
   Add-Content -Value "<tr><td>service</td><td>$currentAppId</td><td><a href='$servicePortalUrl'>TodoListService-OBO-CA</a></td></tr>" -Path createdApps.html
   ...
   # Add Required Resources Access (from 'service' to 'downstreamService')
   Write-Host "Getting access from 'service' to 'downstreamService'"
   $requiredPermissions = GetRequiredPermissions -applicationDisplayName "DownstreamService-OBO-CA" `
   ...
   Write-Host "Creating the AAD appplication (TodoListClient-DotNet-OBO-CA)"
   $clientAadApplication = New-AzureADApplication -DisplayName "TodoListClient-DotNet-OBO-CA" `
                                                  -ReplyUrls "https://TodoListClient-DotNet-OBO-CA" `
                                                  -PublicClient $True
   ...
   Add-Content -Value "<tr><td>client</td><td>$currentAppId</td><td><a href='$clientPortalUrl'>TodoListClient-DotNet-OBO-CA</a></td></tr>" -Path createdApps.html

   $requiredResourcesAccess = New-Object System.Collections.Generic.List[Microsoft.Open.AzureAD.Model.RequiredResourceAccess]
   # Add Required Resources Access (from 'client' to 'service')
   Write-Host "Getting access from 'client' to 'service'"
   $requiredPermissions = GetRequiredPermissions -applicationDisplayName "TodoListService-OBO-CA" `
                                                 -requiredDelegatedPermissions "user_impersonation";
   $requiredResourcesAccess.Add($requiredPermissions)
   Set-AzureADApplication -ObjectId $clientAadApplication.ObjectId -RequiredResourceAccess $requiredResourcesAccess
   Write-Host "Granted."
   # Create the spa AAD application
   Write-Host "Creating the AAD appplication (TodoListSPA-OBO-CA)"
   $spaAadApplication = New-AzureADApplication -DisplayName "TodoListSPA-OBO-CA" `
                                               -HomePage "http://localhost:16969/" `
                                               -ReplyUrls "http://localhost:16969/" `
                                               -IdentifierUris "https://$tenantName/TodoListSPA-OBO-CA" `
   ...
   Add-Content -Value "<tr><td>spa</td><td>$currentAppId</td><td><a href='$spaPortalUrl'>TodoListSPA-OBO-CA</a></td></tr>" -Path createdApps.html
   ...
   ReplaceSetting -configFilePath $configFile -key "ida:CAProtectedResource" -newValue $downstreamServiceAadApplication.IdentifierUris
